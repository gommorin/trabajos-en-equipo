...
/*
    1 abir modal para meter datos del post
    2 mandar los datos del modal a firebase
    3 en la misma función, guardar el id con el que responde firebase en una variable global
    4 usar esa variable global dentro del url para escribir el contenido del post
    5 hacer un patch a esa url con el contenido del post
*/
...
const capturaPost = () => {
    let { title, summary, avatar, user, date, time, cover } = author
    $("#modalPost").empty();
    $("#modalPost").append(`
...
const crearPost = () => {
    $.ajax({ 
        url:`https://ajaxclass9g.firebaseio.com/bell/medium/posts/${postId}.json`,
        method:"PATCH",
        success: ( response ) => {
            console.log( response )
